...
io.sockets.on('connection', (socket) => {
    ...
    socket.on('message', (message) => {
        ...
        const { command, recepient } = message
        ...
        if (recepient === 'ALL') {
            ...
            if (command === 'chat') {
                if (login_ids[recepient]) {
                    io.sockets.connected[login_ids[recepient]].emit('message', message)
    
                    sendResponse(socket, 'message', 200, 'OK')
                } else {
                    sendResponse(socket, 'login', 404, '수신자 ID 를 찾을 수 없습니다.')
                }
            } else if (command === 'groupchat') {
                io.sockets.in(recepient).emit('message', message)
    ...
    socket.on('room', (input) => {
        console.log(`room -> ${JSON.stringify(input)}`)

        const { command, roomId, roomName, roomOwner } = input
